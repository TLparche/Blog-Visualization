...
class TaylorUI:
    ...
    def _layout_axes(self):
        ...
        self.ax_fx = self.fig.add_axes((0.08, 0.26, 0.56, 0.05))
        self.tb_fx = TextBox(self.ax_fx, "f(x) =", initial=self.expr_str)
        self.ax_fx_btn = self.fig.add_axes((0.66, 0.26, 0.10, 0.05))
        self.btn_fx = Button(self.ax_fx_btn, "Apply")

        self.ax_xr = self.fig.add_axes((0.08, 0.20, 0.34, 0.045))
        self.tb_xr = TextBox(self.ax_xr, "x range", initial=str(self.xrange))
        self.ax_yr = self.fig.add_axes((0.44, 0.20, 0.34, 0.045))
        self.tb_yr = TextBox(self.ax_yr, "y range", initial=str(self.yrange))
        self.ax_rng_btn = self.fig.add_axes((0.80, 0.20, 0.14, 0.045))
        self.btn_rng = Button(self.ax_rng_btn, "Set ranges")

        self.ax_a = self.fig.add_axes((0.08, 0.145, 0.18, 0.045))
        self.tb_a = TextBox(self.ax_a, "a", initial=str(self.a))
        self.ax_a_btn = self.fig.add_axes((0.28, 0.145, 0.10, 0.045))
        self.btn_a = Button(self.ax_a_btn, "Set a")

        self.ax_n = self.fig.add_axes((0.08, 0.085, 0.62, 0.04))
